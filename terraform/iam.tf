...
data "aws_iam_policy_document" "instance" {
  ...
  statement {
    ...
    resources = [
      # GetParametersByPath authorizes against the *path being requested*, not
      # the parameters under it, and `prod/*` requires a literal slash after
      # `prod` — so the bare path has to be listed too, or the sweep in
      # render_env_from_ssm.sh is denied and reports "no parameters found".
