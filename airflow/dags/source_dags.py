...
from common import NOTHING_TO_BUILD_EXIT, record_ops_command, run_verdict
...
def _ingest_dag(document, conf):
    ...
    with DAG(
        ...
        description=f"{name} -> raw_{name} -> quality -> transforms -> quality.",
        ...
        verify_raw = BashOperator(
            task_id="verify_raw",
            bash_command=f"set -euo pipefail\ndq run --checkpoint raw_{name}\n",
            execution_timeout=pendulum.duration(minutes=10),
        )
        transform = BashOperator(
            task_id="transform",
            bash_command="set -euo pipefail\nmbx transforms\n",
            # 99 means the manifest is empty. Skip rather than fail: on a fresh
            # checkout there is genuinely nothing to build, and a permanently red
            # run for a known reason is how alerts stop being read.
        ...
        ingest >> verify_raw >> transform >> verify_marts >> record
        [verify_marts, record] >> run_verdict()
    return dag
...
def _backfill_dag(document, conf):
    ...
    with DAG(
        ...
        backfill = BashOperator(
            ...
            bash_command=(
                "set -euo pipefail\n"
                f"ingest run --source {name} --start '{{{{ params.start }}}}' "
        ...
        verify = BashOperator(
            ...
            bash_command=f"set -euo pipefail\ndq run --checkpoint raw_{name} --no-fail-on-error\n",
...
def _reconcile_dag(document, conf):
    ...
    with DAG(
        ...
        reconcile = BashOperator(
            ...
            bash_command=(
                "set -euo pipefail\n"
                f"ingest run --source {name} --start {conf['backfill_start']} "
                "--mark-deleted\n"
            ),
            pool=conf["pool"],
