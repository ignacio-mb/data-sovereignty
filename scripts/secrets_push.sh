...
#
# Read from sources/manifest.json rather than grepped out of the YAML. Two
# reasons, and the second is not hypothetical: `jq` can filter on status, and a
# grep cannot — the reference connector's spec names PYLON_API_KEY, and a grep
# over every spec would make that key mandatory on a stack that never schedules
# it. The manifest is generated by the same parser the runtime uses, so this can
# no longer disagree with what actually runs.
while IFS= read -r token_var; do
  [[ -n "$token_var" ]] || continue
  REQUIRED+=("$token_var")
done < <(
  jq -r '.sources[] | select(.status == "connected") | .token_env' \
    "${REPO_ROOT}/sources/manifest.json" 2>/dev/null | sort -u
)
