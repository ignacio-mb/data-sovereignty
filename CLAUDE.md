...
**Four sources ship connected: Swoogo** (`sources/swoogo.yml`)**, Customer.io**
(`sources/customerio.yml`)**, Lever** (`sources/lever.yml`) **and YouTube**
(`sources/youtube.yml`), because this checkout is also where they are operated
...
every tick. So a fork gets all four whether it wants them or not: **delete the
specs in `sources/` if you are not us**, and the stack goes back to scheduling
nothing.

Lever's `opportunities` — one of only two resources on any shipped spec whose
fetch depends on a persisted incremental cursor rather than a full re-fetch every
...
YouTube is the awkward one, and worth knowing about before touching it because
it breaks two patterns every other spec follows.

**Its credential is not a token.** Every YouTube Reporting API method requires
OAuth 2.0 user-delegated consent; a service account cannot be linked to a
YouTube channel and the device flow is unsupported, so the credential is a
refresh token exchanged for a one-hour access token per run. That is three
secrets against `api.auth`'s single `token_env`, so the spec names an auth type
`runtime._auth()` deliberately refuses (`oauth2_refresh_token`) and the
extension mints the token itself. **That only holds because every one of its 25
resources is delegated** — a resource switched to `full_refresh` would route
through the declarative path, call `_auth()`, and fail on the auth type.
`test_youtube_source.py::TestNothingReachesTheDeclarativePath` pins it, and also
asserts `_auth()` still refuses, so the day the runtime learns this grant the
test is what says the workaround can go. Mint the credential with
`scripts/youtube_oauth_setup.py`; it writes the token into `.env` and never
prints it.

**Its history is 30 days, permanently.** The 20 report tables are bulk CSVs
generated by a *reporting job*, and a job's history reaches exactly 30 days
before the job was created — there is no start/end parameter on any Reporting API
method, so `youtube_backfill` reaches nothing and neither does anything else.
Data not collected inside that window is gone. Its hourly schedule is not about
making daily data finer (YouTube's finest grain is a Pacific-time *day*, ~48h
behind): it is about promptly picking up newly published days **and
restatements**, which arrive as a new report id carrying an old start/end time
and are only visible to a report-creation cursor.

`sources/youtube.yml` carries the rest — the empty-string primary-key columns,
why `date` is text, and the Data API unit budgets that make `comment` and
`caption` converge over days instead of exhausting the project's daily quota in
one run.
...
`sources/` is "we actually run this" — today that is Swoogo, Customer.io, Lever
and YouTube, and `TestWhatThisCheckoutShips` fails until the list and the docs
agree.
